...
const EN = {
  ...
  ,'欢迎来到我的应用':'Welcome to My App','选择左侧预设，拼出你自己的手机界面。':'Choose presets from the left to build your own mobile interface.','看看发现页':'Explore Discover','精选':'Featured','从一个组件开始':'Start with a Component','值得关注的精选内容。':'Curated content worth exploring.','发现灵感':'Discover Ideas','这里可以放你的作品、内容或功能入口。':'Add your work, content, or feature entry points here.','关于我':'About Me','个人资料':'Profile','在这里介绍自己，展示个人资料。':'Introduce yourself and present your profile here.',
  '欢迎来到首页':'Welcome to Home','这里是发现页':'This is the Discover page','这里是我的页面':'This is the Profile page','操作成功':'Action completed','CSS 控制布局、颜色和动画效果。':'CSS controls layout, color, and motion.','HTML 描述页面有哪些元素。':'HTML describes the elements on the page.','JavaScript 负责点击、页面切换与提示。':'JavaScript handles clicks, navigation, and feedback.','点击手机中的任意元素，再点击代码行查看详细中文解释。':'Select any phone element, then select a code line for a detailed explanation.','选择一个组件查看它的样式':'Select a component to inspect its styles','选择组件后查看生成的 HTML':'Select a component to inspect its generated HTML','选择可交互组件后查看点击逻辑。':'Select an interactive component to inspect its click logic.','页面操作按钮':'Page action button','属性与代码已连接':'properties and code are linked','选择画布元素开始理解':'Select a canvas element to start learning','先选择画布元素':'Select a canvas element first','或设置当前页面名称':'or edit the current page name',
  '导航栏背景':'Navigation background','顶部分割线':'Top divider','通栏':'Full-width bar','胶囊':'Pill','填充图标':'Filled icon','浅色底':'Tinted background','下划线':'Underline','外形圆角（px）':'Outer corner radius (px)','底部导航图标':'Bottom navigation icons','上传自己的图标':'Upload your icon','内置图标可改颜色和线条；上传的 SVG、PNG、JPG、WEBP 会随项目保存。':'Built-in icons support custom colors and strokes. Uploaded SVG, PNG, JPG, and WEBP icons are saved with the project.'
  ,'这是一段正文内容，用来说明页面信息。':'This is body text used to explain the page.','继续':'Continue','内容标题':'Content title','简短说明文字':'Short supporting copy','请输入内容':'Enter content','点击修改项目名称':'Click to rename project','项目名称':'Project name','撤销':'Undo','重做':'Redo','重播当前页面动画':'Replay current page animation','页面操作':'Page actions','手机页面':'Phone pages','重命名页面':'Rename page','个当前页面元素':' elements on this page','搜索':'Search','喜欢':'Favorites','音乐':'Music','资料库':'Library','消息':'Notifications','日程':'Calendar','购物':'Shopping','播放':'Play','设置':'Settings','使用首页图标':'Use Home icon','使用发现图标':'Use Discover icon','使用我的图标':'Use Profile icon','使用搜索图标':'Use Search icon','使用喜欢图标':'Use Favorites icon','使用音乐图标':'Use Music icon','使用资料库图标':'Use Library icon','使用消息图标':'Use Notifications icon','使用日程图标':'Use Calendar icon','使用购物图标':'Use Shopping icon','使用播放图标':'Use Play icon','使用设置图标':'Use Settings icon'
};
...
function renderPageManager() {
  ...
  const heading = document.createElement('div'); heading.className = 'page-manager-head'; const name = document.createElement('span'); name.textContent = language === 'en' ? `${state.pages.length} Pages` : `页面 ${state.pages.length}`; const add = document.createElement('button'); add.type = 'button'; add.textContent = language === 'en' ? '+ New Page' : '+ 新建页'; add.onclick = addPage; heading.append(name, add); manager.append(heading);
  const list = document.createElement('div'); list.className = 'page-manager-list';
  state.pages.forEach((page, index) => { const row = document.createElement('div'); row.className = 'page-manager-row'; const select = document.createElement('button'); select.type = 'button'; select.className = 'page-select' + (page === activePage ? ' active' : ''); const pageTitle = language === 'en' ? (EN[state.pageTitles[page]] || state.pageTitles[page]) : state.pageTitles[page]; select.textContent = `${isDetailPage(page) ? '↳' : index + 1 + '.'} ${pageTitle}${isDetailPage(page) ? (language === 'en' ? ' · Detail' : ' · 子页面') : ''}`; select.onclick = () => switchPage(page); select.oncontextmenu = event => { event.preventDefault(); event.stopPropagation(); openContextMenu(event.clientX, event.clientY, 'page', page); }; const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'page-edit'; edit.title = language === 'en' ? 'Rename page' : '重命名页面'; edit.textContent = '✎'; edit.onclick = () => renamePage(page); row.append(select, edit); list.append(row); }); manager.append(list);
}
...
function renderCanvas() {
  ...
  $('#elementCount').textContent = language === 'en' ? `${items.length} elements on this page` : `${items.length} 个当前页面元素`;
  page.scrollTop = scroll;
}

function render() {
  if (!state.pages.includes(activePage)) activePage = state.pages[0];
  $('#projectName').value = language === 'en' ? (EN[state.name] || state.name) : state.name;
...
function renderInspector() {
  ...
  if (selected === '__nav__') {
    const heading = document.createElement('div'); heading.className = 'section-label'; heading.textContent = language === 'en' ? `Bottom Navigation / ${EN[state.pageTitles[activePage]] || state.pageTitles[activePage]}` : '底部导航栏 / ' + state.pageTitles[activePage]; box.append(heading);
  ...
  if (!item) {
    ...
    const shownPageTitle = language === 'en' ? (EN[state.pageTitles[activePage]] || state.pageTitles[activePage]) : state.pageTitles[activePage];
    const title = field(box, '当前页面名称', shownPageTitle); title.onchange = () => { commit(); state.pageTitles[activePage] = title.value.trim().slice(0, 16) || PAGE_LABELS[activePage] || '新页面'; update(); };
...
function showCode(item) {
  let code;
  if (selected === '__header__') code = language === 'en' ? { css: '.phone-header-dot {\n  cursor: pointer;\n}', html: '<button aria-label="Page actions">•••</button>', js: '// Choose a menu, notice, modal, or page navigation action on the right.' } : { css: '.phone-header-dot {\n  cursor: pointer;\n}', html: '<button aria-label="页面操作">•••</button>', js: '// 在右侧选择菜单、提示、弹窗或页面跳转。' };
  else if (!item) code = language === 'en' ? { css: '/* Select a component to inspect its styles */', html: '<!-- Select a component to inspect its generated HTML -->', js: '// Select an interactive component to inspect its click logic.' } : { css: '/* 选择一个组件查看它的样式 */', html: '<!-- 选择组件后查看生成的 HTML -->', js: '// 选择可交互组件后查看点击逻辑。' };
  else { const codeItem = language === 'en' ? { ...item, text: EN[item.text] || item.text, detail: EN[item.detail] || item.detail, prompt: EN[item.prompt] || item.prompt } : item; code = { css: cssFor(codeItem), html: exportElement(codeItem), js: jsFor(codeItem) }; }
  const preview = $('#cssPreview'); preview.replaceChildren();
  const explainLineEnglish = line => {
    const text = line.trim();
    if (!text) return 'This blank line separates the code into easier-to-read sections.';
    const declaration = text.match(/^([\w-]+):\s*(.+);$/);
    const colorMeaning = value => { const [red, green, blue] = hexToRgb(value); return `${value.toUpperCase()} equals RGB(${red}, ${green}, ${blue}). The # marks a hexadecimal color; the six digits represent red, green, and blue.`; };
    if (text.startsWith('/*') || text.startsWith('//') || text.startsWith('<!--')) return `This is a developer comment. The browser does not display it in the phone UI, and removing it will not change the design.`;
    if (/^<\/?[a-z]/i.test(text)) { const tag = text.match(/^<\/?([\w-]+)/i)?.[1] || 'element'; const content = text.match(/>([^<]+)</)?.[1]; return text.startsWith('</') ? `</${tag}> closes the ${tag} element. The slash marks the end of the structure opened by <${tag}>.` : `This line creates a <${tag}> element${content ? ` whose visible content is “${content}”` : ''}. Angle brackets tell the browser that this is page structure.`; }
    if (/^\.[\w-]+\s*\{/.test(text)) { const className = text.match(/^\.([\w-]+)/)?.[1]; return `.${className} selects the current canvas component with class="${className}". The leading dot means “find by class”; the rules inside the braces apply to this component.`; }
    if (declaration) {
      const [, property, value] = declaration;
      if (property === 'color') return `This component currently uses ${value} for its text. ${colorMeaning(value)} The value is applied to the element selected above.`;
      if (property === 'background') return `This component currently uses ${value} as its background. ${colorMeaning(value)} Changing Background in the property panel updates this exact value.`;
      if (property === 'font-size') return `The current text size is ${value}. px means screen pixels; increasing this number makes the text larger and can also increase the component height.`;
      if (property === 'border-radius') return `The current corner radius is ${value}. 0px creates square corners; a value close to half the element height produces a pill shape.`;
      if (property === 'text-align') return `The current value is ${value}, so the content is ${value === 'left' ? 'aligned to the left' : value === 'center' ? 'centered horizontally' : 'aligned to the right'}. The colon separates the property from its value, and the semicolon ends the rule.`;
      if (property === 'opacity') return `The current opacity is ${value}. 1 is fully visible, 0 is invisible, and 0.5 is half transparent.`;
      if (property === 'box-shadow') return `This shadow is currently ${value}. The numbers control horizontal offset, vertical offset, blur, and spread; the final value controls its color.`;
      if (property === 'animation') return `This component currently runs ${value}. The values specify the animation name, duration, speed curve, and how the start and end states are retained.`;
      if (property === 'animation-delay') return `The current delay is ${value}. 0ms starts immediately; 1000ms equals one second.`;
      if (['padding', 'margin', 'gap'].includes(property)) return `${property} is currently ${value}. ${property === 'padding' ? 'It controls the space inside the border' : property === 'margin' ? 'It controls the space outside the component' : 'It controls the space between child elements'}.`;
      if (property === 'display') return `display is currently ${value}, so this component uses ${value === 'grid' ? 'a grid layout' : value === 'flex' ? 'a flexible row or column layout' : value}.`;
      return `The current component sets ${property} to ${value}. The property is on the left, its active value is on the right, and the semicolon finishes the declaration.`;
    }
    if (/addEventListener/.test(text)) return `addEventListener listens for a user action on this component. The 'click' value means that the function runs only when the user clicks it.`;
    if (text === '}' || text === '});') return 'This line closes the current style block or interaction function. It pairs with the opening brace above.';
    return codeMode === 'js' ? `This JavaScript line is part of the component’s real behavior: “${text}”. Parentheses pass values; braces group statements that run together.` : codeMode === 'html' ? `This is the component’s real HTML structure: “${text}”. Tags define the element, while attributes record its current content, state, and purpose.` : `This line contributes to the component’s current appearance: “${text}”. Change the matching property and watch its value update here.`;
  };
  const explainLine = line => {
    if (language === 'en') return explainLineEnglish(line);
    const text = line.trim();
  ...
  code[codeMode].split('\n').forEach(line => {
    ...
    row.onclick = () => {
      ...
      const lineNumber = [...preview.children].indexOf(row) + 1;
      $('#codeLearningNote').textContent = language === 'en' ? `Focused line ${lineNumber}: ${explainLine(line)}` : `已聚焦第 ${lineNumber} 行：${explainLine(line)}`;
    };
    preview.append(row);
  });
  const rawSelectionName = selected === '__header__' ? '页面操作按钮' : item ? TYPE_LABELS[item.type] : '选择画布元素开始理解';
  const selectionName = language === 'en' ? (EN[rawSelectionName] || rawSelectionName) : rawSelectionName;
  $('#codeSelectionLabel').textContent = item || selected === '__header__' ? (language === 'en' ? `${selectionName} · properties and code are linked` : `${selectionName} · 属性与代码已连接`) : selectionName;
  $('#codeExplain').textContent = language === 'en' ? (codeMode === 'html' ? 'HTML describes the elements on the page.' : codeMode === 'css' ? 'CSS controls layout, color, and motion.' : 'JavaScript handles clicks, navigation, and feedback.') : (codeMode === 'html' ? 'HTML 描述页面有哪些元素。' : codeMode === 'css' ? 'CSS 控制布局、颜色和动画效果。' : 'JavaScript 负责点击、页面切换与提示。');
  $('#codeLearningNote').textContent = item ? (language === 'en' ? `“${selectionName}” is selected. Select any code line to see how its current characters and values affect the interface.` : `当前选中“${selectionName}”。点击任意一行代码，可展开这一行的详细中文解释。`) : (language === 'en' ? 'Select any phone element, then select a code line for a detailed explanation.' : '点击手机中的任意元素，再点击代码行查看详细中文解释。');
...
$('#projectName').onchange = event => { commit(); state.name = event.target.value.trim().slice(0, 60) || (language === 'en' ? 'Untitled Project' : '未命名项目'); update(); };
$('#projectName').onkeydown = event => { if (event.key === 'Enter') event.target.blur(); };
