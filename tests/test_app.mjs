...
const theme = fs.readFileSync(root + 'theme.css', 'utf8');
assert.ok(theme.includes('height:min(780px,calc(100dvh - 168px))'));
assert.ok(theme.includes('.workspace{height:calc(100dvh - 64px)'));
assert.ok(theme.includes('.canvas-frame.mobile .canvas-page{min-height:0;overflow-y:auto}'));
